...
(function () {
  ...
  function _buildDOM(container) {
    ...
    // Joystick canvas — always visible (ghost hint at low opacity)
    ...
    Object.assign(_joyCanvas.style, {
      ...
      zIndex:      '25',
      touchAction: 'none',
      display:     'block'   // always shown; opacity controlled in _drawJoystick
    });
    ...
    Object.assign(_fireBtn.style, {
      ...
      borderRadius:  '50%',
      border:        '3px solid rgba(255,40,40,.45)',
      background:    'rgba(255,20,20,.18)',
      color:         'rgba(255,255,255,0.55)', fontSize: '22px',
  ...
  function _gameOver() {
    ...
    if (window.GameRegistry && window.GameRegistry.onGameOver) {
      window.GameRegistry.onGameOver({
        ...
        emoji:     'games/shooting/assets/dog.gif',
        heading:   'Game Over!'
      });
    }
  }
  ...
  function _drawJoystick() {
    ...
    // Idle ghost: always render at min opacity (0.28) as a control hint
    const MIN_OPACITY = 0.28;
    joystick.opacity = joystick.active
      ? Math.min(1,           joystick.opacity + 0.15)
      : Math.max(MIN_OPACITY, joystick.opacity - 0.06);

    const alpha = joystick.opacity;
    // Base position: show ghost at bottom-left when idle
    const bx = joystick.active ? joystick.baseX  : _joyCanvas.width  * 0.2;
    const by = joystick.active ? joystick.baseY  : _joyCanvas.height * 0.75;
    const sx = joystick.active ? joystick.stickX : bx;
    const sy = joystick.active ? joystick.stickY : by;

    _jctx.save();
    // Outer ring
    _jctx.globalAlpha = alpha * 0.3;
    _jctx.beginPath();
    _jctx.arc(bx, by, joystick.baseRadius, 0, Math.PI * 2);
    _jctx.fillStyle = '#ffffff'; _jctx.fill();
    _jctx.globalAlpha = alpha * 0.55;
