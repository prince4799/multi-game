...
(function () {
  ...
  function _dropInterval() {
    // ms per gravity tick — faster than original so auto-fall is clearly visible
    const ms = [400,340,280,220,170,130,100,75,55,40];
  ...
  */
    ...
    if (!_pauseEl) {
      ...
      _pauseEl.innerHTML = `
        <div style="font-family:'Orbitron',sans-serif;font-size:clamp(1.4rem,4vw,2rem);
          font-weight:900;color:#a000f0;text-shadow:0 0 20px #a000f0;
      ...
      if (_canvas && _canvas.parentElement) _canvas.parentElement.appendChild(_pauseEl);
    }
  ...
  window.__onPause  = _pauseGame;
  window.__onResume = _resumeGame;
  ...
  const Tetris = {
    ...
    destroy() {
      ...
      [_loadingEl, _overlayEl, _pauseEl, _canvas].forEach(el => {
